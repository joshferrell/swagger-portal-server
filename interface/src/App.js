...
export default class App extends Component {
    ...
    renderDocument(request) {
        ...
        return <h1>Document does not exist
            <a href="/">Try Another</a>
        </h1>;
    }

    componentDidMount() {
        fetch('http://localhost:8000/docs').then(toJson).then(apis => this.setState({apis})).then(promiseLogger).catch(error => this.setState({message: 'something went wrong'}));
    }
